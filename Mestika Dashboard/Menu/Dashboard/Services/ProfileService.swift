...
class ProfileService {
    ...
    func customerAddress(completion: @escaping(Result<CustomerAddressResponse, ErrorResult>) -> Void) {
        
        guard let url = URL.urlGetCustomerAddress() else {
            return completion(Result.failure(ErrorResult.network(string: "Bad URL")))
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse {
                print("\n\n RESPONSE GET CUSTOMER ADDRESS = \(httpResponse.statusCode)\n")
                
                if (httpResponse.statusCode == 200) {
                    print("OK 200")
                    if let profileResponse = try? JSONDecoder().decode(CustomerAddressResponse.self, from: BlowfishEncode().decrypted(data: data)!) {
